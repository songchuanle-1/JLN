...
class Mono3DRefer_Dataset(data.Dataset):
    def __init__(self, split, cfg, bert_model='bert-base-uncased'):
        ...
        # self.tokenizer = BertModel.from_pretrained(bert_model, do_lower_case=True)
        ...
        self.use_guass = cfg.get('use_guass', False)
        self.sigma = cfg.get('sigma', 10)
        # statistics
    ...
    def scale_and_set_box(self, a, b, c, d, box):
        # 将边界框坐标从(a * b)缩放至(c * d)
        scale_x = c / a
        scale_y = d / b
        # 转换坐标
        ...
        # 创建一个(c * d)的二维数组，并初始化为0
        feature_map = np.zeros((d, c),dtype=np.int64)
        
        # 将边界框内的位置设置为1
        feature_map[y1_scaled:y2_scaled + 1, x1_scaled:x2_scaled + 1] = 1
        if self.use_guass:
            feature_map = self.apply_gaussian_to_mask(torch.tensor(feature_map), self.sigma)
        return feature_map.flatten()
    
    def apply_gaussian_to_mask(self, mask_tensor, sigma):
        # 确保mask_tensor是浮点类型
        ...
        y = y.float()
        # 查找掩码中值为1的像素的位置
        mask_indices = torch.where(mask_tensor == 1)
        # 如果没有找到任何值为1的像素，则返回原始掩码
        if len(mask_indices[0]) == 0:
            return mask_tensor
        # 计算掩码中所有值为1的像素的平均位置作为中心
        center_x = torch.mean(mask_indices[1].float())
        center_y = torch.mean(mask_indices[0].float())
        # 计算每个像素到中心的距离的平方
        distance_squared = ((x - center_x)**2 + (y - center_y)**2)
        # 应用高斯函数
        gaussian_value = torch.exp(-distance_squared / (2 * sigma**2))
        # 将高斯分布应用到掩码中的值为1的像素
        mask_tensor[mask_tensor == 1] = gaussian_value[mask_tensor == 1]
        # print(mask_tensor)
        return mask_tensor
    ...
    def __getitem__(self, item):
        ...
        info = {'img_id': index,
                ...
                'gt_3dbox': [object.h,object.w,object.l,np.float32(object.pos[0]),np.float32(object.pos[1]),np.float32(object.pos[2])] }
        ...
        #### 产生的分隔target
        #### 
        map_size = [[160,48],[80,24],[40,12],[20,6]]
        depth_map_size = [80,24]
        # :
        target_visual_classifier = (self.scale_and_set_box(img_size[0], img_size[1], map_size[i][0],  map_size[i][1], bbox_2d) for i in range(len(map_size)))
        # target_visual_classifier = np.expand_dims(np.hstack(target_visual_classifier), axis=0)
        target_visual_classifier = np.hstack(target_visual_classifier)
        target_depth_classifier = self.scale_and_set_box(img_size[0], img_size[1], depth_map_size[0],  depth_map_size[1], bbox_2d)
        ...
        targets = {
                   ...
                   'word_mask': np.array(word_mask, dtype=int),
                    'text': text,
                    'target_visual_classifier': target_visual_classifier,
                    'target_depth_classifier': target_depth_classifier
                    }
