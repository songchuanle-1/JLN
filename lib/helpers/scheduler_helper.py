...
class PolyLR(_LRScheduler):
    def __init__(self, optimizer, max_iters, power=0.9, constant_ending=0.0, last_epoch=-1):
    ...
    def get_lr(self):
        if self.last_epoch > self.max_iters:
            return [self.constant_ending for _ in self.base_lrs]
        return [(base_lr - self.constant_ending) * (1 - self.last_epoch / self.max_iters) ** self.power + self.constant_ending
                for base_lr in self.base_lrs]
    

def build_lr_scheduler_polyLR(cfg, optimizer, last_epoch):
    
    lr_scheduler = PolyLR(optimizer, max_iters=cfg['max_iters'], power=cfg['power'], constant_ending=cfg['constant_ending'])
