...
class Tester(object):
    def __init__(self, cfg, model, dataloader, logger,loss, train_cfg=None, model_name='mono3dvg',save_featuremap_path=''):
        self.cfg = cfg
        self.model = model
        ...
        self.save_featuremap_path = save_featuremap_path
        # self.save_featuremap_path = save_featuremap_path
        # self.save_featuremap_path = save_featuremap_path
    ...
    def inference(self):
        ...
        for batch_idx, (inputs, calibs, targets, info) in enumerate(self.dataloader):
            ...
            for i in range(batch_size):
                ...
                json_dict = {}
                json_dict['visual']=outputs['feature_map']['visual'][i].squeeze(1).tolist()
                json_dict['depth']=outputs['feature_map']['depth'][i].squeeze(1).tolist()

                file_path = os.path.join(self.save_featuremap_path, f'{key_name}.json')
                try:
                    # 打开文件以写入模式
