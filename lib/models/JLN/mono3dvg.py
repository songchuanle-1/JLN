...
from lib.losses.focal_loss import sigmoid_focal_loss, sigmoid_focal_loss2
...
class Mono3DVG(nn.Module):
    ...
    def forward(self, images, calibs,  img_sizes, text, im_name, instanceID, ann_id):
        ...
        """
        features, pos = self.backbone(images) ###resnet主干网
        ...
        if self.num_feature_levels > len(srcs):
            ...
            for l in range(_len_srcs, self.num_feature_levels):
                if l == _len_srcs:
                    src = self.input_proj[l](features[-1].tensors) ##### 最后一层卷积后形成的 第四层特征
                else:
        ...
        device = srcs[0].device  ###获得设备
        if isinstance(text[0], str):###处理文本信息
            # Encode the text
        ...
        # print(text_memory_resized.size())
        # # pp
        # bs = text_memory_resized.size(0)
        # ### 进行拆分
        # global_visual_feature_list = []
        # aaa_list=[]
        # visual_src_list = srcs ### 按照原始拆分回原始的四层特征
        # cls_visual = self.cls_visual.expand(bs, 1, -1).to(srcs[0].device)
        # for li in range(len(visual_src_list)):
            
        #     # print()
        #     bss,c,h,w = visual_src_list[li].size()
        #     aaa_list.append(visual_src_list[li].view(bss,c,h*w).permute(0,2,1))
        # aaa =torch.cat(aaa_list,dim=1)
        # print(aaa.size())
        # print(111)

        # print(global_visual_feature.size())
        # print(global_text_feature.size())
        ...
        hs, init_reference, inter_references, inter_references_dim, enc_outputs_class, enc_outputs_coord_unact,adapt_visual_src_list,adapt_depth_src_list, global_text_feature, global_visual_feature = self.mono3dvg_transformer(
            srcs, masks, pos, query_embeds, depth_pos_embed,text_memory_resized, text_attention_mask, im_name, instanceID, ann_id)
        ### 将解码后的hs与文本全局特征进行对比损失计算
        #### 
        # decoder_vt_feature = global_visual_feature
        # print(srcs[0].size())
        # global_visual_feature  = self.global_visual_att(
        #     query=cls_visual.transpose(0, 1),
        #     key=visual_align_memory.transpose(0, 1),
        #     value=visual_align_memory.transpose(0, 1)
        # )[0].transpose(0, 1).squeeze(1)
            
            # global_visual_feature_list.append(global_visual_feature)

        # global_visual_feature = torch.cat(global_visual_feature_list,dim=0)
        # print(global_visual_feature.size())
        ...
        out['perd_forback_classifier'] = adapt_visual_src_list
        out['perd_forback_classifier_depth'] = adapt_depth_src_list
        out['decoder_vt_feature'] = global_visual_feature
        out['global_text_feature'] = global_text_feature
...
class SetCriterion(nn.Module):
    ...
    """
    def __init__(self, num_classes, matcher, weight_dict, focal_alpha, losses, contrastive_temperature):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=True):
        ...
        return losses
    def loss_adapt_depth_classifier(self, outputs, targets, indices, num_boxes, log=True):
        """Classification loss (Binary focal loss)
        targets dicts must contain the key "labels" containing a tensor of dim [nb_target_boxes]
        """
        assert 'perd_forback_classifier_depth' in outputs
        src_logits = outputs['perd_forback_classifier_depth']
        bs,n,c = src_logits.size()


        # targets = targets["target_visual_classifier"]
        #### 划分四:
        list_tt = [targets[i]["target_depth_classifier"] for i in range(len(targets))]
        targets_adapt = torch.vstack(list_tt)
        # # 计算损失
        # # 使用 view 将 logits 和 targets 转换为适合 CrossEntropyLoss 的形状
        # logits_reshaped = src_logits.permute(0, 2, 1).contiguous().view(-1, c)
        # targets_flattened = targets_adapt.view(-1)
        # # print(sum(targets_flattened))
        # # print(logits_reshaped)
        # adapt_loss = self.adapt_loss(logits_reshaped,targets_flattened)

        # spatial_shapes = [[160,48],[80,24],[40,12],[20,6]]
        # logits_list = src_logits
        # targets_adapt_list = targets_adapt
        # al = []
        # for i in range(len(spatial_shapes)):
        logits_reshaped = src_logits.squeeze(-1)
        targets_flattened = targets_adapt.to(torch.float32)
        adapt_loss = self.adapt_loss(logits_reshaped,targets_flattened)
        losses = {'loss_adapt_depth': adapt_loss}

        return losses
    ##dd
    def loss_adapt_visual_classifier(self, outputs, targets, indices, num_boxes, log=True):
        """Classification loss (Binary focal loss)
        targets dicts must contain the key "labels" containing a tensor of dim [nb_target_boxes]
        """
        assert 'perd_forback_classifier' in outputs
        src_logits = outputs['perd_forback_classifier']
        bs,n,c = src_logits.size()


        # targets = targets["target_visual_classifier"]
        #### 划分四:
        list_tt = [targets[i]["target_visual_classifier"] for i in range(len(targets))]
        targets_adapt = torch.vstack(list_tt)

        spatial_shapes = [[160,48],[80,24],[40,12],[20,6]]
        logits_list = src_logits.split([H_ * W_ for H_, W_ in spatial_shapes], dim=1)
        targets_adapt_list = targets_adapt.split([H_ * W_ for H_, W_ in spatial_shapes], dim=1)
        al = []
        for i in range(len(spatial_shapes)):
            logits_reshaped = logits_list[i].squeeze(-1)
            targets_flattened = targets_adapt_list[i].to(torch.float32)
            al.append(self.adapt_loss(logits_reshaped,targets_flattened))
        adapt_loss = sum(al)/len(al)
        losses = {'loss_adapt': adapt_loss}
    ...
    def loss_contrastive(self,outputs, targets, indices, num_boxes):
        decoder_vt_feature = outputs['decoder_vt_feature']
        global_text_feature = outputs['global_text_feature'] 
        similarity_matrix = F.cosine_similarity(decoder_vt_feature[:,None],global_text_feature[None,:],dim=-1)
        similarity_matrix /= self.contrastive_temperature
        labels = torch.arange(decoder_vt_feature.size(0)).to(decoder_vt_feature.device)
        loss_vt = F.cross_entropy(similarity_matrix,labels)
        loss_text = F.cross_entropy(similarity_matrix.T,labels)
        losses = {}
        losses['contrastive_learning'] = (loss_vt + loss_text) / 2
        return losses
    ...
    def get_loss(self, loss, outputs, targets, indices, num_boxes, **kwargs):
        ...
        loss_map = {
            ...
            'adapt_visual_classifier': self.loss_adapt_visual_classifier,
            'adapt_depth_classifier': self.loss_adapt_depth_classifier,
            'contrastive_learning':self.loss_contrastive
        }
...
class MLP2(nn.Module):
    ...
    def forward(self, x):
        for i, layer in enumerate(self.layers):
            x =self.sig(layer(x)) if i < self.num_layers - 1 else layer(x)
        return x
def build(cfg):
    # backbone
    ...
    weight_dict['loss_adapt'] = cfg['loss_adapt']
    weight_dict['loss_adapt_depth'] = cfg['loss_adapt_depth']
    weight_dict['contrastive_learning'] = cfg['contrastive_learning']
    ...
    losses = ['labels', 'boxes', 'cardinality', 'depths', 'dims', 'angles', 'center', 'depth_map','adapt_visual_classifier','adapt_depth_classifier','contrastive_learning']
